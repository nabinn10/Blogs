...
<form action="actionblog.php" method="POST" enctype="multipart/form-data">
    ...
    <input type="text" name="title"  class="p-2 rounded block my-4 border w-full" placeholder="Enter Title of Blog" value="<?php echo $row['title'] ?>"]>
   <textarea name="description" class="p-2 rounded block my-4 border w-full" placeholder="Enter Description of Blog"><?php echo $row['description'] ?></textarea>
   <!-- current Picture -->
    <p>Current Picture</p>
    <img src="../uploads/<?php echo $row['photopath']; ?>" alt="currnt available" class="w-20 h-20 object-cover rounded">
    <input type="hidden" name="oldpath"  class="p-2 rounded block my-4 border w-full">
